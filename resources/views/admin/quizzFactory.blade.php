...
@section('content')
    <div class="space-y-8">
        <div
            ...
            <div>
                <div class="flex items-center gap-2 mb-2">
                    <span
                        class="text-[9px] font-mono font-bold uppercase tracking-widest px-2 py-0.5 rounded bg-emerald-500/10 text-yellow-400 border border-yellow-500/20">
                        FACTORY CLUSTER
                    </span>
                ...
                <div class="flex items-center gap-4 font-mono text-[10px] text-zinc-500 uppercase tracking-wider mt-1.5">
                    ...
                    <span class="hidden md:inline">Data Matrix Refresh: <span
                            class="text-zinc-300 font-bold">Real-Time</span></span>
                </div>
            </div>
        ...
        <form action="#" method="GET"
            class="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-zinc-950/20 border border-zinc-900 p-4 rounded-xl">
            <div class="relative">
            ...
            <div>
                <select name="category"
                    ...
                    <option value="security" {{ request('category') == 'security' ? 'selected' : '' }}>Security Protocols
                    </option>
                </select>
            </div>
            ...
            <div>
                <select name="sort_by"
                    class="w-full bg-[#050507] border border-zinc-900 rounded-lg px-3 py-2 text-xs font-mono text-zinc-400 focus:outline-none focus:border-indigo-500 transition-colors"
                    onchange="this.form.submit()">
                    <option value="latest" {{ request('sort_by') == 'latest' ? 'selected' : '' }}>Sort: Latest Commits
                    </option>
                    <option value="questions_high" {{ request('sort_by') == 'questions_high' ? 'selected' : '' }}>Sort:
                        Dynamic High Count</option>
                </select>
            </div>
        </form>
                ...
                <a href="{{ route('show' , $quizz->id) }}"
                    ...
                    <div class="mt-6 pt-4 border-t border-zinc-900/60 flex items-center justify-between relative z-10">
                        ...
                        <div class="flex items-center gap-2">
                            <a href="#"
                                class="p-2 bg-zinc-950 hover:bg-indigo-950/30 border border-zinc-900 hover:border-indigo-900/40 text-zinc-400 hover:text-indigo-400 rounded-xl transition-all duration-150"
                            ...
                            <form action="#" method="POST"
                                onsubmit="return confirm('Are you sure you want to purge this cluster?');" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="p-2 bg-zinc-950 hover:bg-rose-950/30 border border-zinc-900 hover:border-rose-900/40 text-zinc-500 hover:text-rose-400 rounded-xl transition-all duration-150"
        ...
        <div
            ...
            <div class="flex gap-2">
                <a href="#" class="px-3 py-1.5 bg-zinc-950 border border-zinc-900 rounded-lg hover:text-white">Previous</a>
                <a href="#" class="px-3 py-1.5 bg-zinc-950 border border-zinc-900 rounded-lg hover:text-white">Next</a>
            </div>
        </div>
    </div>
@endsection