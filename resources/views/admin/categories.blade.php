...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-left">
            ...
            <div class="p-6 rounded-2xl bg-[#04060c]/30 border border-zinc-900 hover:border-zinc-800/80 flex flex-col justify-between min-h-[220px] transition-all duration-200 group relative overflow-hidden shadow-xl shadow-black/40">
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-center text-[9px] font-mono">
                        <span class="text-indigo-400 uppercase font-bold tracking-wider bg-indigo-500/5 px-2 py-0.5 border border-indigo-500/10 rounded">
                            ID: #{{ sprintf('%02d', $category->id) }}
                        </span>
                        <span class="text-zinc-600 uppercase font-medium tracking-widest">// VEC-0x{{ dechex($category->id + 33953) }}</span>
                    </div>
                ...
                <div class="pt-4 border-t border-zinc-950/80 flex justify-between items-center mt-6">
                    <span class="text-[9px] font-mono text-zinc-600 uppercase tracking-widest">
                        State: <span class="{{ $category->status === 'inactive' ? 'text-zinc-500' : 'text-emerald-500' }} font-bold">{{ strtoupper($category->status ?? 'ready') }}</span>
                    </span>

                    <a href="{{ route('edit-category', $category->id) }}" aria-label="Edit Syntax Array Module"
                        class="p-2 bg-zinc-950 hover:bg-zinc-900 border border-zinc-900 hover:border-zinc-800 text-zinc-500 hover:text-indigo-400 rounded-xl transition-all shadow-md group/btn">
        ...
        @empty
            <div class="col-span-full p-12 text-center bg-[#04060c]/20 border border-dashed border-zinc-900 rounded-2xl space-y-2">
                <span class="text-[10px] font-mono text-zinc-600 uppercase tracking-widest block">// SYSTEM STATE LOG</span>
                <p class="text-sm font-mono text-indigo-400/90">Zero category vectors mapped inside database terminal cluster.</p>
                <p class="text-xs text-zinc-600 max-w-md mx-auto leading-relaxed">
                    The categorization index matrix returned empty values. Initialize a new parameters segment node using the controller action above.
                </p>
            </div>
        @endforelse
    </div>
