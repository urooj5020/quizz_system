        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-zinc-900 pb-6 text-left">
            <div>
                ...
                <div class="flex items-center gap-2 mb-1">
                    <span class="text-[9px] font-mono font-bold uppercase tracking-widest px-2 py-0.5 rounded bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="space-y-6">
                <div class="bg-[#050507]/40 border border-zinc-900 rounded-2xl p-6 space-y-4 text-left">
                    <h3 class="text-xs font-mono uppercase tracking-widest text-zinc-400 font-bold">// Node Configurations</h3>
            ...
            <div class="lg:col-span-2 space-y-4">
                ...
                @forelse ($questions as $index => $question)
                    <div class="bg-[#050507]/40 border border-zinc-900 p-5 rounded-2xl relative overflow-hidden group space-y-4 text-left">
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <div class="flex items-center gap-1.5 opacity-40 group-hover:opacity-100 transition-opacity">
                                <button class="p-1.5 hover:text-indigo-400 transition-colors" title="Edit Question">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </button>
                                <button class="p-1.5 hover:text-rose-400 transition-colors" title="Purge Question">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2">
                            ...
                            <div class="p-3 rounded-xl text-xs font-sans flex items-center justify-between {{ $question->correct_key == 'A' ? 'bg-emerald-950/10 border border-emerald-900/40 text-emerald-400' : 'bg-zinc-950/40 border border-zinc-900 text-zinc-400' }}">
                                <span>A. {{ $question->option_one }}</span>
                                @if($question->correct_key == 'A')
                                    <span class="text-[9px] font-mono font-bold uppercase tracking-widest px-1.5 py-0.5 bg-emerald-500/10 rounded border border-emerald-500/20">CORRECT</span>
                                @endif
                            </div>

                            <div class="p-3 rounded-xl text-xs font-sans flex items-center justify-between {{ $question->correct_key == 'B' ? 'bg-emerald-950/10 border border-emerald-900/40 text-emerald-400' : 'bg-zinc-950/40 border border-zinc-900 text-zinc-400' }}">
                            ...
                            @if($question->option_three)
                                <div class="p-3 rounded-xl text-xs font-sans flex items-center justify-between {{ $question->correct_key == 'C' ? 'bg-emerald-950/10 border border-emerald-900/40 text-emerald-400' : 'bg-zinc-950/40 border border-zinc-900 text-zinc-400' }}">
                            ...
                            @if($question->option_four)
                                <div class="p-3 rounded-xl text-xs font-sans flex items-center justify-between {{ $question->correct_key == 'D' ? 'bg-emerald-950/10 border border-emerald-900/40 text-emerald-400' : 'bg-zinc-950/40 border border-zinc-900 text-zinc-400' }}">
                ...
                @empty
                    <div class="border border-dashed border-zinc-900 p-12 rounded-2xl text-center">
                        <p class="text-xs font-mono text-zinc-600 uppercase tracking-wider">// No question nodes linked into current terminal</p>
                        <a href="{{ route('add-new-question') }}" class="inline-block mt-3 text-xs text-indigo-400 hover:underline font-mono">// Inject First Vector Node</a>
                    </div>
                @endforelse
